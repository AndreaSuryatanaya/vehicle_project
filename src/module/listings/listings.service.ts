...
export class ListingsService {
    ...
    return this.cache.getOrSet(
      ...
      async () => {
        ...
        const filters: ListingSearchFilterRecord = {
          ...
          cursor: query.cursor
            ? this.decodeSearchCursor(query.cursor)
            : undefined,
        };
  ...
  async findSuggestions(query: string): Promise<ListingSuggestion[]> {
    ...
    return this.cache.getOrSet(
      ...
      60,
      () => this.listingsRepository.findSuggestions(query),
    );
  }

  findAllFilterOptions(): Promise<FilterOptionCount[]> {
    return this.cache.getOrSet('listings', 'filter-options', 300, () =>
      this.listingsRepository.findAllFilterOptions(),
    );
  }
  ...
  async findCategoryFilters(
    ...
    return this.cache.getOrSet(
      ...
      300,
      () => this.listingsRepository.findCategoryFilters(categoryId),
    );
  }

  async findOne(id: string): Promise<ListingRow> {
    return this.cache.getOrSet('listings', `detail:${id}`, 60, async () => {
  ...
  async remove(
    id: string,
  ): Promise<{ id: string; status: string; deletedAt: Date }> {
  ...
  private parseFilters(query: ListingBrowseQuery): ListingFilterRecord {
    ...
    const cursor = query.cursor
      ? this.decodeCursor(query.cursor, limit)
      : undefined;
    ...
    if (error instanceof DatabaseError) {
      if (error.code === '23503') {
        throw new BadRequestException(
          'A referenced category, model, or attribute does not exist',
        );
      }
      if (
        error.code === '23514' ||
        ...
        throw new BadRequestException(
          'Listing contains an invalid field value',
        );
      }
      if (error.code === '23505') {
        throw new BadRequestException(
          'Listing images contain a duplicate position',
        );
      }
    }
    throw error;
